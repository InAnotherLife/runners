...
class Competitors(Settings):
    ...
    def gen_competitors(self):
        """
        Генерирует данные о спортсменах: номер, фамилия, имя. В методе
        происходит чтение данных из файла с фамилиями и именами спортсменов,
        получение номеров спортсменов, определение количества мужчин и женщин.
        Затем для каждого номера спортсмена выбирается случайное фамилия и имя.
        Данные добавляются в словарь __competitors под соответствующим номером
        спортсмена.
        arg: None
        return: None
        """
